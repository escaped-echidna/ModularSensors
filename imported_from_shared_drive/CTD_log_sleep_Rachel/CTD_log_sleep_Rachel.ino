...
const char *LoggerID = "Waikato-Troy_B";
// How frequently (in minutes) to log data
const uint8_t loggingInterval = 1; //set to 1 minute for testing pj
// Your logger's timezone.
const int8_t timeZone =8;  // Auckland :note timezone wont take 2 digit number pj 
...
const int8_t wakePin = A7;        // MCU interrupt/alarm pin to wake from sleep A7
// Set the wake pin to -1 if you do not want the main processor to sleep.
// In a SAMD system where you are using the built-in rtc, set wakePin to 1
const int8_t sdCardPin = 12;      // MCU SD card chip select/slave select pin (must be given!)
const int8_t sensorPowerPin = 22; // MCU pin controlling main sensor power 22 (-1 if not applicable)
...
void setup()
{
    ...
    if (getBatteryVoltage()< 3.0)    //set for PS1270-12V7.0Ah battery 25/6/19 pj
    {
